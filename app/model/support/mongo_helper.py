...
def mongo_item_to_python_type(mongo_item):
    if mongo_item is None:
        ...
        return mongo_set_to_python_list(mongo_item)

    elif isinstance(mongo_item, dict):
        if mongo_item.get('type') == 'Point':
            return mongo_item['coordinates']

    elif isinstance(mongo_item, Document) or isinstance(mongo_item, EmbeddedDocument):
        return mongo_doc_to_dict(mongo_item)

    else:
        return mongo_item


def mongo_doc_to_dict(doc, exclude_fields=list()):
    if doc is None:
        return None
